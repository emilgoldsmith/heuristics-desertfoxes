...
class Model(object):
    def __init__(self, sess, checkpoint_path, total_weights, restore=False):
        ...
        layer_size_input = 115 - (25 - total_weights) * 2
        layer_size_hidden = 20
        layer_size_output = 1
...
with sess.as_default(), graph.as_default():
    try:
        total_weights = int(os.sys.argv[1]) if int(os.sys.argv[1]) else 25
    except:
        total_weights = 25

    model = Model(sess, model_path, summary_path, checkpoint_path, total_weights, restore=True)

################################## NOT AT ALL COPY PASTED #########################################
...
while 1:
    ...
    if features == "done":
        break
    features = np.array(map(float, features.split(' '))).reshape(1, -1)
    evaluation = model.get_output(features)
    print evaluation
    write(str(evaluation))
